...
export class PokemonesComponent implements OnInit {
  ...
  getPokemons() {
    this.http.get<any>('https://pokeapi.co/api/v2/pokemon?limit=30').pipe(
      map((data: any) => data.results.map((pokemon: any) => pokemon.url))
    ).subscribe(
      (pokemonUrls: string[]) => {
          ...
          (pokemonDataArray: any[]) => {
            for (const pokemonData of pokemonDataArray) {
              this.http.get<any>(pokemonData.species.url).subscribe(
                ...
                error => console.error('Error al obtener detalles del Pokémon', error)
              );
            }
          },
